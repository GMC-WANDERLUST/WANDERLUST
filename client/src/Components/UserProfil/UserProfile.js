  ...
  return (
    <React.Fragment>
      ...
      <div key={user._id}>
        ...
        <ModalEditFirstName action={"Edit First Name"} data={user.FirstName} />
        <h1>{user.LastName}</h1>
        <ModalEditLastName action={"Edit Last Name"} data={user.LastName} />
        <h2>{user.Country}</h2>
        <h3>
          {user.DayOfBirth} {user.MonthOfBirth} {user.YearOfBirth}
        </h3>
        <section>
          <input type="button" value="Add Post" onClick={openPostModal} />
          <ModalAddPost />
        </section>
        <article>
          {userPost.map((post) => (
